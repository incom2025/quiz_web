...
# Просте "сховище сесій" у пам'яті процесу (для Render/Railway вистачає на 1 інстанс)
# Якщо треба 100% стабільність між рестартами/кількома інстансами — перенесемо в DB.
...
def export_results_to_xlsx(xlsx_path: str = "results.xlsx") -> str:
    # Експорт у Excel на запит (щоб не було конфліктів при паралельних записах)
    ...
    with sqlite3.connect(DB_FILE) as con:
        rows = con.execute("SELECT ts, surname, name, grp, score, total FROM results ORDER BY id DESC").fetchall()
...
def start(
    ...
    # Важливо: ми НЕ перемішуємо відповідь правильно/неправильно ключем,
    # а прив’язуємо правильний варіант до літери A/B/C/D (як у CSV).
    # Варіанти можна перемішувати, але тоді треба мапити ключ — тут робимо прозоро:
    # показуємо як є (A/B/C/D), щоб не було помилок.
...
def quiz(request: Request, session_id: str):
    ...
    # Таймер на клієнті: скільки залишилось
...
def submit(request: Request, session_id: str, answers: List[str] = Form(default=[])):
    ...
    questions = sess["questions"]
    # answers приходять як список значень "A"/"B"/"C"/"D" (або порожньо)
    # Порядок — як у питаннях.
    score = 0
    normalized = []
    for i, q in enumerate(questions):
        ...
        if a not in {"A", "B", "C", "D"}:
            a = ""
        normalized.append(a)
        if a and a == (q["Prav_vid"] or "").strip().upper():
            score += 1

    db_insert_result(sess["surname"], sess["name"], sess["grp"], score, len(questions))

    # Прибираємо сесію (щоб не накопичувалась)
